...
Примечание по стартовой базе (2026-05-06): для MVP0 уже зафиксирован операционный baseline на VPS (`Cloud.ru + Docker Compose + Caddy HTTPS`). Это используется как переходный контур до полного target-production в managed cloud.
...
### Weeks 6-7: Identity + Voice Hardening
- Закрепить social auth контур (Google/Yandex OAuth) в production: redirect URIs, error-handling, security review.
- Унифицировать голосовой канал: переход от browser-only voice input к управляемому SpeechKit STT/TTS baseline.
- Добавить observability по voice/auth шагам (success/error rate, latency).
...
- `docs/HISTORY/MVP1_PRODUCT_PLATFORM_PLAN.md` (этот документ).
