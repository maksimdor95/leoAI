...
Рекомендуемая воронка Jack: 1→2→3→4/5→7→8→9→11→12→13.  
Хвост улучшения матча: 15→16 (или 17).  
WannaNew: …→9→18.

**Отвал на лендинге (главная метрика для маркетинга):**

| Смотреть | Где | Смысл |
| -------- | --- | ------ |
| Цель `landing_viewed` | Цели → Лендинг | Сколько **зашли** на главную |
| Цель `landing_cta_clicked` | Цели → Клик CTA | Сколько **нажали** «Начать» |
| Разница viewed − CTA | те же отчёты за период | Сколько **отвалились на лендинге** без клика |
| Воронка шаг 1 → 2 | Отчёт «LEO — Jack» | То же в % |

Не смотрите только воронку «с конца»: прямые заходы в `/chat` не дают шаг 1. Для отвала на лендинге сравнивайте **две цели**, не «прошёл ли я весь Jack».
