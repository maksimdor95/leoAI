...
> **v1.4 (2026-06-30):** маршрут по **этапам**, не по календарным дням; дни — слой удержания.  
> См. [`INTERVIEW_PREP_CJM_v1.4.md`](./INTERVIEW_PREP_CJM_v1.4.md), [`INTERVIEW_PREP_VALUE_AND_IA.md`](./INTERVIEW_PREP_VALUE_AND_IA.md).  
> Полный merge v1.4 в этот файл — отдельный шаг после утверждения.
...
| v2 | Сравнение «было / стало» при повторной подготовке (§13.4) | ✅ `priorPrepSnapshot` + секция в PDF |
...
> Актуальный план закрытия пробелов: [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)

| Требование методологии | Статус | Действие / фаза |
|------------------------|--------|-----------------|
| Вкладка «Подготовка» как маршрут | ✅ | B2: «Сегодня», прогресс, чеклисты (§6) |
| Learn без вопроса в теории | ✅ | A1 + UI: `theory_learn`, кнопка «Готов к проверке» |
| Две персоны LEO | ✅ | A3 `buildInterviewCorePersona(mode)` + phase personas |
| PACK после диагностики | ✅ | A2 `diagnostics_pack` + C3 Pack UI на сцене |
| Один вопрос в диагностике | ✅ | mode protocol; pack после 4 ответов |
| Уроки в плане дня | ✅ | B1 `PrepActivity`, панель «Сегодня» |
| Матрица seniority × фазы | ✅ | A4 + `buildSeniorityIntensityPrompt` (§1.5) |
| Протокол Rescue | ✅ | B4 бейдж + C3 `rescue_cheatsheet` в артефактах |
| Ритуалы мока M0/M2 | ✅ | B4: кнопка «Начать мок», `mockPhase`, debrief |
| PDF-отчёт | ✅ | C2: §9, шпаргалки из `prepArtifacts` |
| PDF v2 «было / стало» | ✅ | `priorPrepSnapshot` + секция 1.1 в PDF (§9.5) |
| Pack в UI (артефакты) | ✅ | C3: `packType`, секция «Артефакты» |
| `employer_questions` Pack | ✅ | `employer_questions_pack` + `employerQuestionsPackComplete` |
| prepPlan × seniority (AI) | ✅ | `buildPrepPlanSeniorityBlock` в generate-prep-plan |
| `prep_complete` | ✅ | C1: карточка + чеклист + PDF CTA |
| Role packs (11 семейств) | ✅ | §12: все кроме generalist |
| Merge 3 prompt-spec в один doc | ⚪ | Опционально |

**Легенда:** ✅ готово · 🟡 частично (backend/UI) · ❌ не начато
...
| Engineering | `engineering_systems` | Backend, Frontend, Fullstack, SRE, ML Engineer | ✅ | backend, developer, system design… | — (готово) |
| QA / Testing | `qa_quality` | QA, QA Lead, SDET, Test Automation | ✅ | qa, sdet, тестиров, автотест… | — (готово) |
| Sales / BD | `sales_commercial` | AE, SDR, BDR, Account Manager, KAM | ✅ | sales, sdr, account executive… | — (готово) |
| Operations / Delivery | `operations_delivery` | Project Manager, Scrum Master, PMO, Release | ✅ | project manager, scrum, delivery… | — (готово) |
| Leadership / Mgmt | `leadership_behavioral` | Team Lead, Head of, Director, VP | ✅ | lead, director, head… | — (готово) |
| Design / UX | `design_ux` | UX/UI, Product Designer, Researcher | ✅ | designer, ux, figma, usability… | — (готово) |
| Marketing | `marketing_growth` | Performance, CRM, Brand, Content | ✅ | marketing, seo, growth marketing… | — (готово) |
| Support / Success | `customer_success` | CSM, Support Lead, Onboarding | ✅ | csm, customer success… | — (готово) |
| HR / Recruiting | `hr_people` | Recruiter, HR BP, Talent | ✅ | recruiter, talent acquisition… | — (готово) |
| Generalist | `generalist` | Склад, админ, редкие роли, смешанные | ⚪ | fallback | по запросу |

\*Детекция — `inferRoleTrack()` в `interviewPrepPrompts.ts`. Порядок: **marketing** → analytics → design → product → qa → engineering → sales → customer success → operations → hr → leadership → generalist.
...
#### C. Sales / Commercial (`sales_commercial`)

| Поле | Содержание |
|------|------------|
| **Статус** | ✅ Pack готов (фаза D P1) |
...
| **Marketing** | Воронка, каналы, attribution, креатив vs data | Кейс: план кампании | ✅ P3 |
...
> Детальный план с эпиками, DoD и сроками: [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)
...
### 13.4. Повторное использование (2-я вакансия) — v1.1 ✅

| Вопрос | Решение v1.1 |
|--------|--------------|
| Новая сессия или продолжение? | **Новая сессия** на вакансию |
| Переиспользуются STAR? | `starBank` на `userId` (Redis) + в `collectedData`; режим STAR предлагает адаптацию |
| История grading | `priorFatalGaps` → в промпт `prepPlan` |
| Диагностика при 2-й вакансии | **Сокращённая** (2 ответа вместо 4) при том же role track |
| PrepPlan | Генерируется заново с `prepContext` |
| UI | `PrepRetentionPanel` во вкладке «Подготовка» |

**Минимум v1.1:** при старте 2-й вакансии LEO явно говорит: «Вижу, вы готовились к {role A}. Для {role B} построю новый план; ваши STAR из прошлой сессии могу помочь переупаковать.»
...
- План внедрения: [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)
- UI план: `frontend/components/chat/PrepPlanCard.tsx`
- Вкладка: `frontend/app/chat/page.tsx` (`sidePanelTab === 'profile'`)

---

*Документ подлежит версионированию. Изменения — через PR с обновлением §11 gap analysis, §12.3 role packs, §13 roadmap и [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md).*
...
| 1.3 | §11 gap sync с кодом; ссылка на IMPLEMENTATION_PLAN.md |
| 1.4 | Фаза 0 + A1–A4 backend: теория learn/check, diagnostics pack, персоны, seniority |
| 1.5 | Фазы B–C: Подготовка, гейт мока, prep_complete, PDF, Pack UI (C3) |
| 1.6 | §11 gap sync; `buildSeniorityIntensityPrompt`; Pack-карточки на сцене |
| 1.7 | Фаза D P0: `engineering_systems` + `qa_quality` full packs × 6 режимов |
| 1.8 | Фаза D P1: `sales_commercial` + `operations_delivery` full packs × 6 режимов |
| 1.9 | Фаза E: 2-я вакансия, STAR bank, сокращённая диагностика, `PrepRetentionPanel` |
| 2.0 | Фаза D P2: `leadership_behavioral` + `design_ux` full packs × 6 режимов |
| 2.1 | Фаза D P3: `marketing_growth` + `customer_success` + `hr_people` full packs |
| 2.2 | PDF v2 «было/стало»; `employer_questions` Pack; prepPlan × seniority |
