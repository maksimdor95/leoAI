/**
 * Normalize vacancy card fields for UI (Leo chat cards).
 * Goal: light but domain-clear titles (like HH), strip TG pitch/noise — not erase context.
 */
...
/** «Role в Компанию» — team/domain after «в» is filtered via isTeamOrgPart(). */
...
/** Domain / team context that must stay in the title. */
function isDomainRest(rest: string): boolean {
  const r = rest.trim();
  if (!r) return false;
  if (/^\(/.test(r)) return true; // (B2B, …)
  if (/^в\s+/i.test(r) && isTeamOrgPart(r.replace(/^в\s+/i, ''))) return true;
  return false;
}

/** Short brand/name suffix after role — safe to drop from title (org lives in company). */
...
export function cleanJobTitle(raw: string, org?: string | null): string {
  ...
  // «Product в Авито» — never treat «в команду / отдел …» as company.
  ...
  if (org) {
    ...
    if (orgC && text.toLowerCase().endsWith(orgC.toLowerCase())) {
      ...
      const keptAsCompanyCue = /\sв$/i.test(prefix); // «Product в Авито»
...
export function guessOrgFromTitle(raw: string, existingOrg?: string | null): string | null {
  ...
  if (existing && !looksLikeChannelHandle(existing)) return existing;
...
export function looksLikeChannelHandle(company: string): boolean {
  const c = company.trim();
  if (!c || /\s/.test(c)) return false;
  return /^[A-Za-z][A-Za-z0-9_]{2,40}$/.test(c);
}

export function normalizeVacancyCardFields(input: {
  title: string;
  company: string;
  source?: string | null;
  ...
  if (source.startsWith('tg_')) {
    const handle = source.slice(3);
    if (!company || company.toLowerCase() === handle || looksLikeChannelHandle(company)) {
      company = guessOrgFromTitle(rawTitle, null) || 'Telegram';
    } else {
      company = guessOrgFromTitle(rawTitle, company) || company;
    }
  } else if (!company || looksLikeChannelHandle(company)) {
    company = guessOrgFromTitle(rawTitle, company) || company || 'Компания не указана';
  }
