...
Если в «Контекст рынка» есть missingSkillsTop — упомяните 1–2 навыка из списка как пробелы к закрытию (не выдумывайте навыки вне списка).
Добавьте 2–3 коротких next_actions на ближайшие 7 дней (что сделать в профиле/поиске). Без URL, без названий платных курсов, без процентов и вероятностей оффера.
Верни JSON: { "market_fit_summary": "...", "next_actions": ["...", "..."] }
Пример тона: «На рынке сильный спрос на product leadership. Ваш опыт в крупных IT-компаниях делает вас конкурентоспособным кандидатом. Рекомендуем вам смотреть роли Head of Product, Product Lead или CPO. Имеет смысл явно добавить в профиль навыки из пробелов выдачи.»`;
  }
